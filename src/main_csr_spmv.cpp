...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource fill_buffer(ocl::getFillBuffer());
    ...
    for (auto [min_nnz_per_row, max_nnz_per_col] : evaluated_min_max_nnz_per_row) {
        ...
        for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
            ...
            fill_buffer.exec(gpu::WorkSize(GROUP_SIZE, nrows), output_vector_values_gpu, 0, nrows);

            t.restart();

            // Запускаем кернел, с указанием размера рабочего пространства и передачей всех аргументов
            // Если хотите - можете удалить ветвление здесь и оставить только тот код который соответствует вашему выбору API
            ocl_spvm.exec(gpu::WorkSize(GROUP_SIZE, csr_values.size() * GROUP_SIZE), csr_row_offsets_gpu, csr_columns_gpu, csr_values_gpu, vector_values_gpu, output_vector_values_gpu, nrows, ncols, csr_values.size());
